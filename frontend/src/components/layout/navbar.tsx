    ...
    <div className="relative" ref={ref}>
      <button
        ...
        className={`flex items-center justify-center w-8 h-8 rounded-[10.5px] transition-colors cursor-pointer
          ${open
            ? "bg-[#5d5d5a]/10 text-[#5d5d5a]"
            : "text-[#5d5d5a]/70 hover:bg-[#5d5d5a]/10 hover:text-[#5d5d5a]"
          }`}
      >
...
export function Navbar({ activeView, onViewChange }: NavbarProps) {
    ...
    const navItems: { label: "Kanban" | "Calendar"; icon: typeof Kanban; href: string }[] = [
      { label: "Kanban", icon: Kanban, href: "/planner" },
      { label: "Calendar", icon: Calendar, href: "/planner" },
    ];
    ...
    return (
      ...
      >
        {/* ── Logo ── */}
        <Link href="/planner" className="flex items-center gap-1.5 shrink-0 group">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {!isSettings && (
            <>
              ...
              <div className="flex items-center gap-1.5">
                {[
                  { icon: Timer, onClick: () => {} }, 
                  { icon: Settings, onClick: () => router.push("/settings"), active: isSettings }
                ].map((item, idx) => (
                  <button
                    key={idx}
                    type="button"
                    onClick={item.onClick}
                    className="flex items-center justify-center w-8.5 h-8.5 bg-[#F8F6F5] rounded-[8px] text-[#5d5d5a]/70 hover:bg-gray-50 hover:text-[#5d5d5a] transition-allcursor-pointer"
                  >
          ...
          {/* User Menu dengan Bordered style */}
